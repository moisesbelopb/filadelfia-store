...
import { type DeliveryReportRow, getDeliveryReport } from "@/lib/queries/admin";
...
export default async function EntregasPage({
  ...
  const { rows, totals } = await getDeliveryReport({ start: range.start, end: range.end });

  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex flex-col gap-1">
          <p className="eyebrow">Controle de entregas</p>
          <h1 className="section-title">Entregas realizadas</h1>
          <p className="text-sm text-muted-foreground">
            {range.label} · {totals.count} {totals.count === 1 ? "entrega" : "entregas"}
          </p>
        </div>
        <PrintButton />
      </div>
      ...
      <PeriodFilter
        basePath="/admin/entregas"
        active={range.period}
        from={range.from}
        to={range.to}
      />

      {/* Resumo — o valor que importa é o total de taxas a repassar ao motoboy. */}
